...
std::string getDeviceName(const std::string& input) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if(search_device){
                if(devInfo.ipAddress[0] == '\0'){
                  size_t found = std::string(search_device).find(":");
                   if (found != std::string::npos) {
                        if(strncmp(devInfo.displayName, search_device, strlen(search_device)) == 0){
                            target_device_info = devInfo;
                        }
                   }else{
                        if(strncmp(getDeviceName(devInfo.displayName).c_str(), search_device, strlen(search_device)) == 0){
                            target_device_info = devInfo;
                        }
                    }
                }else {
